        ...
        {menuList.map(({ dropDownMenu = [], id, label, path }) => {
          return (
            <li key={id} className="leading-[164%] relative group">
              <Link
                to={path}
                className="font-semibold text-lg font-poppins group-hover:text-primary-foreground transition-all duration-500 py-5 flex items-center gap-1"
              >
                {label}
                  ...
                  {dropDownMenu.map(({ id, label, path }) => {
                    return (
                      <li key={id}>
                        <Link
                          to={path}
                          className="font-semibold font-poppins hover:text-cream-foreground hover:bg-primary transition-all duration-500 py-3 px-2.5 block border-b border-b-slate-300"
                        >
                          {label}
                        </Link>
                      </li>
